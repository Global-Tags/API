...
module.exports = {
    ...
    async execute(interaction, message, member, user) {
        ...
        if(!player.watchlist) return interaction.reply({ embeds: [new EmbedBuilder().setColor(bot.colors.error).setDescription(`❌ This player is not on the watchlist!`)], ephemeral: true });

        player.watchlist = false;
        player.save();

        interaction.reply({ embeds: [new EmbedBuilder().setColor(bot.colors.success).setDescription(`✅ The player is not being watched anymore!`)], ephemeral: true });
    }
}