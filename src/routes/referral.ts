...
export default new Elysia({
    ...
    if(executor.referred) return error(403, { error: i18n(`referral.alreadyReferred`) });
    
    player.referrals.push({ uuid: session.uuid, timestamp: Date.now() });
    await player.save();

    executor.referred = true;
    executor.save();
