import { verify } from "jsonwebtoken";
import AuthProvider from "../AuthProvider";
import { stripUUID } from "../../game-profiles";

const PUBLIC_KEY = '-----BEGIN PUBLIC KEY-----\\n-----END PUBLIC KEY-----';

type LabyConnectPayload = {
    client_id: string,
    uuid: string
}

export default class LabyConnectProvider extends AuthProvider {
    constructor() {
        super('LabyConnect');
    }

    async getUUID(token: string): Promise<string | null> {
        try {
            const payload: LabyConnectPayload = verify(AuthProvider.trimTokenType(token), PUBLIC_KEY) as LabyConnectPayload;
            if(payload.client_id != 'labyconnect' || !payload.uuid) return null;

            return stripUUID(payload.uuid);
        } catch(err) {
            return null;
        }
    }
}