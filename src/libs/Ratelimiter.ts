...
export default class Ratelimiter {
    ...
    public ratelimitResponse({ set, status, ip, i18n }: IpContext): any {
        ...
        if(ratelimitData.limited) return status(429, { error: i18n('$.error.ratelimit').replaceAll('<seconds>', String(Math.ceil(ratelimitData.reset / 1000))) });
    }
