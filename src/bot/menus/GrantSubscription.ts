...
export default class GrantSubscription extends SelectMenu {
    ...
    async selection(interaction: StringSelectMenuInteraction, message: Message, values: string[], member: GuildMember, user: User) {
        ...
        try {
            ...
            interaction.reply({ embeds: [new EmbedBuilder().setColor(colors.success).setDescription('✅ The subscription was successfully granted!')], flags: [MessageFlags.Ephemeral] });
        } catch(err: any) {
            interaction.reply({ embeds: [new EmbedBuilder().setColor(colors.error).setDescription(`❌ Entitlement could not be created: ${(err as DiscordAPIError)?.message || 'Unknown error'}`)], flags: [MessageFlags.Ephemeral] });
        }
    }
}